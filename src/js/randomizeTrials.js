...
export default (exp, agentsSingle, targetsSingle) => {
  ...
  const possiblePositionsCoords = (exp.subjData.studyversion === 'hedge') ? positionsSingleContinuous : positionsSingleBoxes;
  // ...same as positionsSingleContinuous.length or positionsSingleBoxes.length
  const possiblePositionsNr = (exp.subjData.studyversion === 'hedge') ? bins : exp.trials.boxVersion;
